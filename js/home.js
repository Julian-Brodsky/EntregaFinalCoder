...
if (document.URL.includes("home.html")) {
  ...
  function borrarCard(indice) {
    ...
    // Guardar el índice en una variable global para usarlo en la función eliminarMovimiento
    window.indiceAEliminar = indice;
  }
  
  function eliminarMovimiento() {
    let indice = window.indiceAEliminar;
    movimientos.splice(indice, 1);
    localStorage.setItem("movimientos", JSON.stringify(movimientos));
    historial = JSON.parse(localStorage.getItem("movimientos"));
    agregarMovimiento();
  
    // Cerrar el popup después de eliminar el movimiento
    let popup = document.getElementById("confirmPopup");
    popup.style.display = "none";
  }
  
  function cerrarPopup() {
    // Cerrar el popup sin eliminar el movimiento
    let popup = document.getElementById("confirmPopup");
    popup.style.display = "none";
  }
}
